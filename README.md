...
- Redis Cache
    ...
    - cache 에 저장된 데이터를 역직렬화 하는 과정에선 Jackson 에서 LocalDateTime 을 제대로 변환할 수 없는 문제점이 있어 object mapper 에 관련 설정을 추가해주는 것으로 해결하였습니다.
    

권순형

- jwt 필터 적용
    - Controller 부분에 필터를 적용하면 필터에 해당 하는 멤버나 해당 role을 가지고 있어야 기능을 실행할 수 있도록 하였습니다.
    - 토큰을 가지고 있을 경우에만 기능이 실행하게 되었습니다.
- GlobalExeption 생성
    - 제대로 된 동작 수행이 이뤄지지 않을 경우에 오류처리가 아닌 예외처리를 만들기 위해 GlobalExeption을 만들었습니다,
- 토큰 생성
    - 프론트에서 회원정보를 저장할 수 있는 토큰을 생성하여, 회원정보를 공유할 수 있게 하였습니다.
- member CRUD 생성
    - 회원가입, 로그인, ADMIN계정이 아닌 계정에 권한 부여, 유저 조회를 할 수 있는 CRUD를 생성하였습니다.
- 내용설명 적용
    - jwt 필터를 적용할 경우 생기는 예외처리 오류인 모든 예외를  “JWT verification failed” 로 인식하는 오류를 다시 일일이 예외처리하였습니다.
    - 누구인지에 따라 권한을 부여하고 권한에 따라 기능을 주었습니다.
